---
name: inventory
description: 盤點這台電腦上所有 AI agent（Claude Code、Codex、Antigravity、Cursor、OpenClaw、Hermes）的全域規則、全域技能、專案規則、專案技能，寫摘要並用本機網站呈現。總入口，會依序執行 inventory-setup → inventory-scan → inventory-summarize → inventory-serve。使用者說「盤點規則與技能」「看我有哪些 skills」「agent inventory」「/inventory」時使用。
---
...
flowchart LR
    ...
    B --> C[inventory-summarize<br/>為缺摘要的檔案撰寫中文摘要]
    C --> D[inventory-serve<br/>起本機網站並開瀏覽器]
```
...
4. 執行 `inventory-serve` 技能。
5. 回報：各工具四分類的數量、跨工具共用的數量、網址。
